...
template <typename T> using Vec = std::vector<Ciphertext<T>>;
template <typename Element> using CC = CryptoContext<Element>;
template <typename Element> using CT = Ciphertext<Element>;
template <typename Element> using CTVec = Vec<Element>;
