...
% Within the context of a Residue Number System (RNS), the \gls{crt} provides the mathematical foundation for representing large integers as arrays of smaller, independent residues. By choosing word-sized coprime moduli $q_i$, operations on a large integer modulo $Q$ can be executed component-wise in parallel across the individual moduli, completely bypassing multi-precision carry propagation. Equation~\eqref{eq:crt-gadget} explicitly formulates this reconstruction as an inner product, which maps naturally to the gadget reconstruction structures utilized in homomorphic encryption.
