...
%% CRT
%%
\section{The Chinese Remaineder Theorem}
\label{app:crt}
...
\begin{theorem}[Negacyclic Convolution Theorem]
    ...
    By exploiting the symmetry of roots of unity via fast decimation algorithms---functionally identical to the Fast Fourier Transform (FFT) utilizing the Cooley-Tukey butterfly structure---the computational evaluation of both the forward and inverse NTT is mathematically reduced from a standard complexity of $\mathcal{O}(N^2)$ to $\mathcal{O}(N \log N)$. This precise algorithmic reduction forms the absolute baseline requirement for practical latency in RNS-variant homomorphic cryptography.
\end{theorem}