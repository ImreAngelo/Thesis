...
\begin{algorithmic}[1]
    ...
    \For{$i \gets 0$ \textbf{to} $\ell - 1$ \textbf{in parallel}}
        \State $s_i \gets \Call{RightShift}{c',\; i \cdot b}$ \Comment{Slice out the $i$-th bit-window}
        \State $u_i \gets \Call{And}{s_i,\; \text{mask}}$ \Comment{Extract the intermediate unsigned digit}
        % \If{$i + 1 < \ell$}
