...
The decomposition on the gadget $g = (1, B, \dots, B^{\ell-1})$ is essentially digit extraction. The classical approach is to compute standard unsigned base-$B$ digits, producing a decomposition vector where each digit $d_i \in [0, B)$. Under our abstract gadget decomposition definition~\autoref{sec:gadget-decomposition}, the quality of this decomposition is $\beta = B - 1$.
...
\newpage
Once we have a working $\Call{Decompose}{}$ function, the external product can be written as \autoref{alg:external-product}. Our implementation is functionally the same, except that it has been split into three stages for better parallelization. First, all the $2k$ inverse \gls{ntt} evaluations required by the decompositions are calculated in parallel. Second, the decompose calls are merged into the external product function, and the two ciphertext elements are decomposed in parallel over $2k\ell$ threads. This stage includes the $2k\ell$ forward \gls{ntt} calls. Finally, the for loop calculating the inner product is parallelized over $\ell$ threads.
