...
\begin{itemize}
    \item As remarked in \autoref{}, the HPS gadget is equivalent to limb look-up on RNS polynomials: $\hps{P}(a) \cong a\cdot I_k$
    \item \autoref{} also shows we can use the standard BV gadget on each limb of an RNS polynomial.
    \item The resulting gadget contains mostly 0's, and the structure is known.
\end{itemize}
...
\subsubsection{RGSW-BV Data Structure}
Each RGSW ciphertext is stored as a vector \texttt{std::vector<DCRTPoly>} of $2\cdot k\cdot \ell$ RLWE ciphertexts (\texttt{DCRTPoly}), where $k$ is the number of moduli in the RNS basis $\mathcal{B}_Q$ and $\ell$ is the decomposition parameter. 
Let $g_k = ()$ and $g'_\ell = (1, B, \dots, B^{\ell-1})^T$ be the gadget vector of the BV decomposition. Then an RGSW ciphertext encrypting a plaintext $\mu \in R_t$ 
\begin{equation}
    C = Z + \begin{pmatrix}
        [\mu]_{q_0} \cdot g' & 0 \\
        \vdots & \vdots \\
