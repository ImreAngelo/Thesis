...
\begin{conjecture}[Mixed Product]
    ...
    and observe that a sequence of fresh hybrid-RGSW ciphertexts $H_i = \Call{HGSW}{m_i}$ can be applied using \emph{only} hybrid external products. We conjecture that this product inherits the additive noise growth of the internal product (\cref{sec:results:noise}), only with the smaller per-step noise of the hybrid external product, such that:
    \begin{equation*}
