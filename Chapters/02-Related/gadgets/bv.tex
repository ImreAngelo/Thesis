...
%% TODO: Clean up, make sure \hat{q} notation is known --- have it listed as notation somewhere or refer back to base conversion e.g. recall \hat{q} from {ref}
By stating the \gls{crt} as an inner product \eqref{eq:crt-gadget}, it is immediately clear that \gls{crt} with moduli $q_i$ is an exact decomposition on $g = (\hat{q}_0\cdot\hat{q}_0^{-1}, \dots, \hat{q}_{k-1}\cdot\hat{q}_{k-1}^{-1})$ with $\beta = \max_i\|q_i\|_\infty/2$ (assuming centered residues) and $\epsilon = 0$. This leads to... % somehow include HPS 
%
