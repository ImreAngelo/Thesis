...
\begin{remark}
    The factors $[(\frac{Q}{q_i})^{-1}]_{q_i}$ and $\hat{q}_i =\frac{Q}{q_i}$ can be pre-calculated and stored in memory (discussed in \autoref{sec:impl:core}).
\end{remark}

\subsubsection{Scale}
Note that the error $\alpha_xQ \pmod{QP}$ produced by \autoref{alg:fast_base_extension} would be completely removed if we multiplied by $P$. This observation leads to the specialized operation of ``\textit{extend and scale by} $P$''. I refer to this operation as $\Call{Scale}{x, P} \mapsto [xP]_{\mathcal{B}_{QP}}$.
\begin{gather*}
