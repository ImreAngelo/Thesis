...
\begin{definition}[Second RNS form (Double CRT/NTT/Evaluation Form)]
...
The naive approach to base conversion reconstructs $a$ from its residues through inverse \gls{crt}, and then reduces the result modulo each modulus of the new basis. This reintroduces the multi-precision arithmetic that RNS was meant to avoid. A cheap but approximate alternative exists, described next.
...
\begin{equation}
    ...
    \mathsf{ApproxModDown}(x) = \left[P^{-1}\left(x - \mathsf{FastBConv}([x]_{\mathcal{B}_P}, \mathcal{B}_P, \mathcal{B}_Q)\right)\right]_{\mathcal{B}_Q}
\end{equation}
