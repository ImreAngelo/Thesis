\section{Noise Management Techniques in BGV-RNS} % Different Variants of Key Switching
\begin{itemize}
...
\begin{lemma}[Layered Gadget Decomposition]
    ...
    Gadget decompositions can be layered.
...
Digit decomposition cannot be applied directly the \gls{rlwe} sample in an \gls{rns} scheme, as the system only has access to the residues. When Bajard et. al. first implemented the BV cryptosystem using RNS \cite{cryptoeprint:2016/510} they exchanged digit decomposition for CRT decomposition, using the  
...
\begin{itemize}
    \item Note on HPS gadget = RNS limb lookup $\to$ \autoref{sec:impl:lib}
    \item HPS is exact but noise is large ($\beta = Q/2,\; \epsilon = 0$); can apply digit decomposition to each residue polynomial as per \autoref{thm:gadget-layers}, lowering $\beta \to B/2$ where $B = \lfloor \max{q_i}/\ell \rfloor + 1 \implies \epsilon = 0$
\end{itemize}
...
\begin{itemize}
    ...
    \item Generate the key in $QP$ as $(a\cdot s + \Delta P + \epsilon, a) \in \mathcal{B}_{QP}^2$
...
\begin{itemize}
    ...
    \item GHS is a single-digit decomposition $(\ell = 1)$ with qualtiy $\approx Q$
