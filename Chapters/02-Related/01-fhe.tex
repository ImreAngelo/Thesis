...
With few exceptions, all of the best performing FHE frameworks \cite{cryptoeprint:2011/277, BFV, CKKS, TFHE} rely on the hardness of the \gls{lwe} problem first introduced in \cite{regev:2009}, or variants thereof. 
...
\begin{definition}[Centered Residue Representation]
    Mathematically the two groups are isomorphic, we are simply renaming $q/2 +1 \mapsto -q/2$ etc., but in practice this affects noise bounds by keeping $\|\epsilon\| \leq q/2$ instead of $\|\epsilon\| \leq q$.
    \begin{align}
        [a]_q &= (a + q/2 \bmod{q}) - q/2 \;\longmapsto\; [-q/2, q/2) \\
        \mathbb{Z}_q &= \{-q/2, -q/2 + 1, \dots, -1, 0, 1, \dots, q/2 - 1\}
    \end{align}
\end{definition}

\begin{itemize}
    \item Concretely state what happens when $2 \not| q$ for example $\mathbb{Z}_5 = \{-2, -1, 0, 1, 2\}$ is not really $\pm5/2 = 2.5$
\end{itemize}

%%
%% R_Q
\begin{definition}[Canonical Form]
    We are interested in polynomials in the ring $R_Q = \mathbb{Z}_Q[X]/(X^N+1)$ with degree (up to) $N - 1$ and coefficients in $[-Q/2, Q/2)$ 
    %, and different representations of this polynomial. The canonical form \eqref{form:canonical} of such a polynomial $a$ is the form most is familiar with.
    \begin{equation}
...
\begin{definition}[RLWE ciphertext] $\vec{x} = (b,a) \in R_q^2$ containing a uniformly random \textit{public element} $a$ and the masked element $b = a\cdot s + \Delta m + \epsilon$ derived from a persistent secret key $s$ and randomly sampled noise $\epsilon$, where $\Delta$ is the message scaling factor and $m\in R_t$ is the plaintext message being encrypted.
    ...
    An RLWE ciphertext $\vec{x}$ encrypting a message $m$ under the secret key $s$ is denoted $\rlwe_s(m)$ unless the secret key is obvious from context, then $\rlwe(m)$ is sufficient.
\end{definition}

\begin{definition}[RLWE Public Key]
    A public key is an encryption of 0 $p = (a\cdot s + \epsilon, a)$ so that encryption using the public key is as simple as adding the encoded message into 
\end{definition}
...
\begin{itemize}
    ...
    \item We mix BGV with TFHE-style external products to achieve multiplication (of binary digits), and avoid any HE multiplication.
\end{itemize}
...
\begin{definition}[Modulus Switching]
    Modulus switching is an operation transforming a ciphertext $\vec{x}\in R_{Q}^{2}$ encrypted under a large modulus $Q$ into a new ciphertext $\vec{x}^{\prime}\in R_{Q^{\prime}}^{2}$ defined under a smaller modulus $Q^{\prime} < Q$, while guaranteeing that both ciphertexts decrypt to the identical underlying plaintext message. This can be achieved by scaling the ciphertext by a factor of $Q^{\prime}/Q$ and rounding the result to the nearest integer.
    \begin{equation}
        x\in R_{Q'}^{2} = (b', a') = (\lfloor b\frac{Q}{Q'} \rceil, \lfloor a\frac{Q}{Q'} \rceil)
    \end{equation}
\end{definition}

In leveled FHE schemes, modulus switching functions as the primary mechanism for noise management. Because scaling down the ciphertext modulus induces a proportional reduction in the associated noise, the procedure permits continuous homomorphic computations without incurring the significant performance penalty associated with bootstrapping.
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
While most of the theory covered throughout this thesis is scheme-agnostic, the implementation is built on top of the BGV scheme \cite{cryptoeprint:2011/277}, \textcolor{red}{chosen because} ... which places the message in the \textit{least} significant bits of the ciphertext by encoding the message $v$ as a plaintext $m$ via
...
\begin{definition}[SIMD Batching] 
    If the chosen plaintext modulus $t$ is a prime satisfying the algebraic condition $t \equiv 1 \pmod{2N}$, then BGV supports per-slot SIMD packing \cite{cryptoeprint:2011/133}. This mode converts a plaintext from a polynomial of order $N - 1$ into $N$ independent constants $[c_i]_t$. Homomorphic operations are applied to each slot individually, for example $(1, 2) \cdot (2, 3) = (2, 6)$ instead of $(1, 2) \cdot (2, 3) = (1 + 2x)(2 + 3x) = (2, 7, 6)$.
\end{definition}