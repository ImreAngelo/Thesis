...
\textcolor{red}{Informal:} Due to the nature of modular arithmetic, addition/subtraction of two polynomials in $R_Q$ is equivalent to adding/subtracting their CRT forms (modulo $p_i$). As each element fits within a computer word, each pair of elements can be added/subtracted in parallel.
...
\begin{remark}
\textcolor{red}{Rather confusingly, OpenFHE calls this form \emph{Coefficient} format. Both D/CRT forms are represented by an \texttt{DCRTPoly} object, so a polynomial in CRT form is represented by a \texttt{DCRTPoly} object in \texttt{Format::Coefficient} mode.}
\end{remark}
