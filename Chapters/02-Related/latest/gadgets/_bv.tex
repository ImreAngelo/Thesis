...
Although we cannot use \textit{digit} decomposition, RNS is itself based on a different decomposition: \textit{CRT} decomposition. This means we can decompose an RNS polynomial using the RNS primes themselves as a basis (\textcolor{red}{where the $\mathcal{B}_Q$ notation usually comes from!} as shown in \cite{cryptoeprint:2016/510, cryptoeprint:2021/204}. % At level $1$ (s.t. $Q = Q_i$ in the notation of \cite{cryptoeprint:2021/204}) this becomes:
\begin{align}
    \label{eq:bv:d}
...
It was later shown by Halevi, Polyakov and Shoup \cite{cryptoeprint:2018/117} that the factors can be moved into $P_Q(a)$ without violating \autoref{lemma:bv_gadget}. Since $[(Q/q_i)^{-1}]_{q_i}[Q/q_i] = 1$ in tower $j = i$ and 0 in towers $j\not= i$, this optimization effectively makes the decomposition free as $P_Q(a) \cong a_{\mathsf{limbs}}\cdot I_k$ where $k$ is the number of RNS limbs.

\begin{align*}
    D_Q(a) &= \left( \imod{a}{0}, \imod{a}{1}, \cdots, \imod{a}{i}\right)\\
    P_Q(a) &= \left( a\cdot\imod{\left(\frac{Q}{q_0}\right)^{-1}}{0} \cdot \qmod{\frac{Q}{q_0}}, \dots, a\cdot\imod{\left(\frac{Q}{q_i}\right)^{-1}}{i} \cdot \qmod{\frac{Q}{q_i}} \right)
\end{align*}

% \noindent\textcolor{red}{Note that $[(Q/q_i)^{-1}]_{q_i}[Q/q_i] = 1$ in tower $j = i$ and 0 in towers $j\not= i$. See \autoref{sec:impl:hps} for why this is especially useful.}
...
By \autoref{the:gadget}, we can use these gadget vectors to define an external product. (Also optimized heavily in \autoref{sec:impl:hps}.) However, the noise added scales with $\sum_{i}q_i/2 \leq k\cdot \max_i\{q_i\}/2$ (\textcolor{red}{Cite error bounds from TFHE paper}). This is fine when only applied once, for example in keyswitching, but quickly becomes unmanageable when using \textit{internal} products \eqref{def:internal_product}.

To combat this, we can add a second layer of decomposition using the classic BV technique with $g = (1, B, \cdots, B^{\ell-1})$. I started this section by saying this method is slow, and that still rings true as digit decomposition cannot be performed in RNS/DCRT mode. However, due to the gadgets, the coefficients of $a$ \textbf{are} the coefficient/canonical representations of the smaller polynomials $a\pmod{q_i}$. By applying the technique to each RNS digit individually, we:
\begin{enumerate}
    \item avoid multi-precision arithmetic even in $D_Q(a)$
    \item convert to RNS coefficient mode instead of canonical (standard coefficient)
    \item can apply $P_Q(a)$ fully in evaluation mode
\end{enumerate}
...
\begin{corollary}[Correctness of Double Decomposition]
    Applying digit decomposition to each element of $\hps{P}(a)$ individually yields a valid external product by \autoref{the:gadget}.

    \textcolor{red}{Does not matter wether the decomposition is first or last $Decomp(P_{HPS}(a))$ or $P_{HPS}(Decomp(a))$}

    \begin{proof}
        Let $\bv{P}(a) = \hps{P}(a\cdot g)$ and $D_{BV}(a) = D_{HPS}(a\cdot g^{-1})$ where $g = ()$ and $g^{-1}$ is the digit extraction of $n$ such that $g^{-1}(n)\cdot g = n$. Using the fact that $P_{HPS}(\cdot), D_{HPS}(\cdot)$ satisfies the exact gadget property \eqref{property:gadget}, we see that the same is true for $P_{BV}(\cdot), D_{BV}(\cdot)$:
        \begin{gather}
...
\ifnotes
    ...
    \input{Chapters/02-Related/latest/gadgets/hps-extra}
    ...
    \input{Chapters/02-Related/latest/gadgets/powers-of-base}
