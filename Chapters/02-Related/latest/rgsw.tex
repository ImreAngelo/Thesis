...
\begin{theorem}%[Generalized Gadget RGSW]
    ...
    Any pair $D(a), P(b) \in \mathbb{Z}_{Q}^{\ell}$ that satisfies the approximate gadget property \eqref{def:approx_gadget_prop} defines an RGSW ciphertext \eqref{def:rgsw} and a valid external product \eqref{def:external_product} such that $\mathsf{RLWE}(a)\boxdot\mathsf{RGSW}(b) = \mathsf{RLWE}(a\cdot b)$. 
    ...
    \begin{proof}
        1. The inexact gadget property holds for $G= I_2\otimes P(1)$ and $G^{-1}(x) = [D(x_0)|D(x_1)]$.
        \begin{align*}
        ...
        2. If the inexact gadget property holds, then the external product is valid.
        \begin{gather*}
        ...
        \begin{gather*}
            ...
            \end{array}
            \right) \\
            \text{Let }a_z = \sum_{i=0}^{2\ell - 1}g^{-1}_ia_i \text{, and observe that the first element becomes} \\
        % \end{gather*}
        % \begin{gather*}
